...
# TruckDrive scene, and print how to open it from your laptop over an SSH tunnel.
#
# The viewer renders: camera frustums (RGB) with optional in-image lidar projection + cuboid
# edge overlay; all lidar point clouds (colorizable / fusable); 3D wireframe cuboids; and the
# rig trajectory. (Radar is not rendered -- the viewer has no radar component yet.) It runs a
# web server on THIS node; you tunnel its port to your laptop.
#
...
#
# Usage:
#   tools/data_converter/truckdrive/visualize_scene.sh
#   OUTPUT_DIR=/lustre/.../ncoreV4/truckdrive SCENE_NAME=scene_28_3 PORT=8090 \
#     tools/data_converter/truckdrive/visualize_scene.sh
...
# Pick the scene: explicit SCENE_NAME, else the first converted scene_* dir.
...
--- Once the browser is open (verify everything visually) ---
  * Sequence tab : scrub the Reference Frame slider / Play to step through the ~26s scene;
                   toggle Rig Trajectory + Show Rig Frame.
  * Lidars tab   : enable each of the 4 lidars (3 Ouster + Aeva); try Color Style =
                   Range/Height/Timestamp; turn on Fuse to accumulate -- a coherent road/scene
                   = poses + extrinsics OK.
  * Cameras tab  : see the 11 RGB frustums (pinhole + fisheye). In Overlay Settings, turn on
                   "Project Lidar" and "Overlay Cuboids" -- if the projected points + box edges
                   line up with the image, the extrinsics/intrinsics/poses are all correct.
  * Cuboids tab  : 3D boxes should sit on the lidar points of real objects. NOTE: cuboids are
                   labelled only in the CENTRAL ~20s (first ~3s and last ~3s of the scene have
                   sensor frames but no boxes), so boxes vanish near both ends -- expected.
==============================================
